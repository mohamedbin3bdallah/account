<!-- footer content -->
	...
	<script>
		function mytotal()
		{
			...
			var price = submit_form.elements['values[]'];
			var total = 0;
			...
			if(total != salary)
			{
				$("#total").css("background-color", "#F5A9A9");
				document.getElementById('totalMessage').innerHTML = 'يجب ان يكون مجموع مفردات المرتب تساوي المرتب';
				$("#submit").attr('disabled', 'true');
			}
			else
			{
				$("#total").css("background-color", "#eeeeee");
		...
		$(document).ready(function(){
			$('.add').click(function () {
				$('.mytable').append('<tr><td><input type="text" name="salaryitems[]" class="form-control" required="required"></td><td><input type="text" name="values[]" class="form-control" placeholder="00.00" maxlength="10" oninput="mytotal()" required="required"/><?php echo ' '.$system->currency; ?></td><td style="text-align:center;"><i class="delete glyphicon glyphicon-remove"></i></td></tr>');
			});
		});
