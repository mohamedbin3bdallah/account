    ...
    <script src="<?php echo base_url(); ?>gentelella-master/build/js/custom.min.js"></script>
		...
		/*$(document).ready(function(){
			$("#type").change(function() {
				var val = $(this).val();
				if(val) window.location = "<?php echo base_url(); ?>items/getmodels/"+val;
				else window.location = "<?php echo base_url(); ?>items/add/";
			});
		});*/
...
$(document).ready(function(){
    $("#type").change(function(){
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url(); ?>items/getmodels',
			data: {
				'val':val,
			},
			success: function (response) { document.getElementById("model").innerHTML = response; }
		});
	});
});
	</script>
