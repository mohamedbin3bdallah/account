        ...
        <div class="right_col" role="main">
          <div class="">
			...
			<?php
				...
				echo form_open_multipart('paymentvouchers', $attributes);
			?>
			<div class="row">
					...
					<table id="datatable-buttons" class="table table-striped table-bordered bulk_action">
					  ...
					  <?php foreach($paymentvouchers as $paymentvoucher) { ?>
                        <tr class="<?php if($paymentvoucher->endtime < time() && $paymentvoucher->accept != '1') echo 'danger'; ?>">
						  ...
						  <td width="10%"><?php echo ArabicTools::arabicDate($system->calendar.' Y-m-d', $paymentvoucher->pvtime).' , '.date('h:i:s', $paymentvoucher->pvtime); if(date('H', $paymentvoucher->pvtime) < 12) echo ' '.lang('am'); else echo ' '.lang('pm'); ?></td>
						  <td width="10%"><?php echo ArabicTools::arabicDate($system->calendar.' Y-m-d', $paymentvoucher->endtime).' , '.date('h:i:s', $paymentvoucher->endtime); if(date('H', $paymentvoucher->endtime) < 12) echo ' '.lang('am'); else echo ' '.lang('pm'); ?></td>
						  <td><?php echo $paymentvoucher->pvquantity; ?></td>
						  <td><?php echo $paymentvoucher->item.' ( '.$paymentvoucher->model.' - '.$paymentvoucher->type.' )'; ?></td>
						  <td><?php echo $paymentvoucher->user; ?></td>
						  <td>
