	<?php
		...
		$html .= '
                          <table>
							  ...
							  <tr>
                                <th>'.lang('day').'</th>
                                <td>'.date('Y-m-d', $order['btime']).'</td>
								<th>'.lang('time').'</th>
                                <td>'.date('h:i:sa', $order['btime']).'</td>
                              </tr>
							  <tr></tr>
                              <tr>
                                <th>'.lang('customer').'</th>
		...
		if(!empty($order['items']))
		{
			$html .= '
                          <table>
                            ...
                            <tbody>
			';
			for($it=0;$it<count($order['items']);$it++)
			{
				$html .= '
							<tr>
                                <td width="15%">'.$order['items'][$it]['icode'].'</td>
                                <td width="39%">'.$order['items'][$it]['item'].$order['items'][$it]['joitem'].'</td>
                                <td width="17%">'.$order['items'][$it]['quantity'].'</td>
                                <td>'.$order['items'][$it]['price'].' '.$system->currency.'</td>
                            </tr>
				';
			}
			$html .= '
		...
		$mpdf->AddPage();
		$mpdf->WriteHTML($html);
		$mpdf->Output('bill_'.$order['code'].'_'.date('Y-m-d-H-i-s', time()).'.pdf', 'I');
	?>